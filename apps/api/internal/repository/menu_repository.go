...
func (r *postgresMenuRepository) List(ctx context.Context, branchID int64) ([]model.MenuItem, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Name, &item.Category, &item.StorePrice, &item.StorePriceAvailable, &item.LinemanPrice, &item.LinemanPriceAvailable, &item.CostPrice, &item.LinemanCostPrice, &item.Status, &item.ImageURL, &item.PreparationSteps, &ingredients, &linemanIngredients); err != nil {
			return nil, err
		}
