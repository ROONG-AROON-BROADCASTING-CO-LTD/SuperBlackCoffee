...
func TestInventoryExpiryStatusBoundaries(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "sixty days away is expiring soon", expiryDate: datePointer(time.Date(2026, time.November, 11, 0, 0, 0, 0, time.UTC)), want: "expiring_soon"},
		{name: "more than sixty days away is not expiring soon", expiryDate: datePointer(time.Date(2026, time.November, 12, 0, 0, 0, 0, time.UTC)), want: "none"},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := inventoryExpiryStatus(test.expiryDate, now, 60); got != test.want {
